...
independent evidence source by default.
...
\item \textbf{Independence assumption.} Top-level branches are assumed
  conditionally independent given $H$ and $\neg H$ by default.  When
  branches share a common methodological flaw, use
  \texttt{correlation\_group} and \texttt{rho} to model the dependence
  explicitly.  The adversarial correlation attack
  (Section~\ref{sec:adversarial}) can quantify the impact of any remaining
