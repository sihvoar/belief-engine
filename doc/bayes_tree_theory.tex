...
Belief Engine (Python package: \texttt{bayes\_tree}), a framework for evaluating
complex hypotheses by decomposing them into hierarchical evidence trees.
Top-level branches act as independent---or explicitly correlated---evidence
...
Belief Engine is not an AI that thinks for you---it is a calculator for belief
revision that forces you to make your assumptions explicit.  The tool combines
Bayesian inference with Monte Carlo simulation to propagate uncertainty through
a tree of evidence.  Each branch of the tree represents a piece of evidence
characterized by a \emph{likelihood ratio} (LR), and the tool computes a
posterior probability distribution for the root hypothesis.
...
interpretable tree of how local evidence shifts beliefs.  Evidence branches that
share a common methodological flaw can be tagged with a
\texttt{correlation\_group} and correlation coefficient $\rho$, so that the
engine models their dependence via a Gaussian copula rather than assuming
independence.
...
The tool computes this by collecting all \emph{leaf nodes} from the tree and
summing their log-LRs against the root prior.  Internal (non-leaf) nodes are
pure groupers and carry no evidence.  The conditional independence assumption
applies to leaf nodes given $H$ and given $\neg H$; see the warnings below.

\subsection{Correlated Evidence (Correlation Groups)}

When two or more branches share a common methodological flaw---for example,
radiocarbon dates from the same laboratory with the same contamination
source---the independence assumption is violated.  The tool supports explicit
modelling of such dependencies via \texttt{correlation\_group} tags.

Branches sharing a group are sampled using a \emph{Gaussian copula}:
\begin{enumerate}
\item Generate independent standard normals $Z_1, \ldots, Z_k$ for the $k$
  grouped branches.
\item Correlate them: $Z'_j = \rho Z_1 + \sqrt{1-\rho^2}\, Z_j$ for $j \geq 2$,
  where $\rho$ is the user-specified correlation coefficient.
\item Transform to uniform quantiles via $U_i = \Phi(Z'_i)$.
\item Use $U_i$ as the quantile input for each branch's LR sampler (inverse CDF).
\end{enumerate}

The effect is that positively correlated branches tend to produce extreme LR
values together, \emph{widening} the posterior distribution and yielding more
conservative credible intervals.  Non-grouped branches remain independent.
...
\textbf{Interpretation.}  The resulting distribution is the distribution of the
posterior \emph{point estimate} induced by uncertainty in the user-specified LR
intervals and distribution families.  It is \emph{not} a hierarchical Bayesian
model over the likelihood ratios themselves---the LR intervals are treated as
fixed inputs, and Monte Carlo sampling propagates that interval uncertainty
into a distribution over possible posteriors.
...
\item[beta] --- Beta distribution scaled to the interval, controlled by
  ...
  \end{equation}
  \textbf{Caveat:} this scaling is \emph{linear}.  On a wide interval such as
  $[0.01, 100]$, a linearly scaled Beta puts almost all probability mass near
  \texttt{lr\_min} in log terms, which is rarely the analyst's intention.
  Prefer \texttt{log\_uniform} or \texttt{log\_beta} for wide intervals.

\item[log\_beta] --- Beta distribution in log-space.  The Beta variate is used
  to interpolate between $\ln(\text{lr\_min})$ and $\ln(\text{lr\_max})$:
  \begin{equation}
  \text{LR} = \exp\!\Big(\ln(\text{lr\_min}) + \text{Beta}(\alpha,\beta)
    \times \big(\ln(\text{lr\_max}) - \ln(\text{lr\_min})\big)\Big)
  \end{equation}
  This gives the analyst shape control (via $\alpha$, $\beta$) while respecting
  the multiplicative nature of likelihood ratios.
\end{description}

%═══════════════════════════════════════════════════════════════════════════════
\section{Tree Structure and Aggregation}

The tree has two kinds of nodes:
\begin{itemize}
\item \textbf{Leaf nodes} carry evidence: each specifies a likelihood ratio
  or interval.  Only leaves contribute to the posterior via the log-odds sum.
\item \textbf{Internal nodes} are pure groupers.  They must not carry
  \texttt{lr\_min}, \texttt{lr\_max}, or \texttt{likelihood\_ratio}.  The
  parser rejects any node that has both children and an LR parameter.
\end{itemize}

The root prior is the \emph{only} prior.  The posterior is computed as a flat
traversal over all leaves:
\begin{equation}
\text{lo}(\text{posterior}) = \text{lo}(\text{prior}_{\text{root}})
  + \sum_{\ell \in \text{leaves}} \ln(\text{LR}_\ell)
\end{equation}

Internal nodes' contributions are computed for display only: the
``subtree contribution'' of an internal node is the combined LR of its
descendant leaves, showing how each evidence group influences the overall
posterior.  These figures are informational and do not feed back into the
computation.

\paragraph{Leave-one-out ranking.}
Removing an internal node removes all its descendant leaves from the sum.
Removing a leaf removes just that leaf.  The resulting change in the baseline
posterior quantifies each node's importance.
...
Each child node is either a \textbf{leaf} (carries an LR) or an
\textbf{internal grouper} (has children but no LR).
...
- node: "Evidence description"
  ...
  lr_dist: log_uniform      # Distribution: log_uniform | uniform | beta | log_beta

  # Optional for beta distribution
  lr_alpha: 2               # Beta shape parameter alpha
  lr_beta: 2                # Beta shape parameter beta

  # Optional: correlation with other branches
  correlation_group: c14    # Group tag (branches with same tag are correlated)
  rho: 0.7                  # Pearson correlation coefficient (0 to 1)
\end{lstlisting}

\paragraph{Internal node (grouper):}
...
- node: "Evidence category"
  ...
  children:                 # Leaf children carry the actual evidence
    - node: "Sub-evidence A"
      ...
      lr_max: 3.0
      evidence_type: for
    - node: "Counter-argument"
      lr_min: 0.3
      lr_max: 0.7
      evidence_type: against
\end{lstlisting}

\textbf{Rule:} A node with \texttt{children} must not specify
\texttt{lr\_min}, \texttt{lr\_max}, or \texttt{likelihood\_ratio}.  The
engine raises a \texttt{ValueError} if this constraint is violated.
...
\begin{itemize}
\item \textbf{Independence assumption.} Leaf nodes are assumed
  conditionally independent given $H$ and $\neg H$ by default.  Sibling
  leaves that are facets of the same underlying observation
  (e.g.,\ three laboratories running C14 on samples from the same cloth)
  violate this assumption.  Mitigations: (a) merge such leaves into one;
  (b) keep only the strongest; or (c) use \texttt{correlation\_group} and
  \texttt{rho} to model the dependence explicitly.  The adversarial
  correlation attack (Section~\ref{sec:adversarial}) can quantify the
  impact of any remaining unmodeled correlations.

  \textbf{Latent common causes} are the most dangerous form of dependence
  because the analyst may not be aware of them.  Examples:
  \begin{itemize}
  \item Multiple ``independent'' eyewitnesses who all heard the same
    earlier rumor before testifying---their testimonies are correlated
    through the rumor, not through the event itself.
  \item Multiple published studies that drew from the same underlying
    patient registry or shared dataset---their results are correlated even
    if the authors were unaware of each other.
  \item Two archaeological dating methods that rely on the same calibration
    curve share a systematic error component.
  \end{itemize}
  No \texttt{correlation\_group} can capture a dependence the analyst does
  not know about.  We recommend running the adversarial correlation attack
  as a routine sanity check: if injecting $\rho = 0.5$ between any pair of
  strong branches causes a large posterior shift, the conclusion is fragile
  and the independence of those branches should be explicitly justified.

\item \textbf{Leaves-only evidence.} Only leaf nodes carry likelihood ratios;
  internal nodes are pure groupers.  A YAML file with an LR on a node that
  also has children is rejected at parse time.  This design eliminates
  double-counting that arises from evidence appearing both at the parent
  and child level.
...
\item \textbf{Tree view:} A recursive collection routine computes
  subtree-contribution statistics for internal nodes (their descendant
  leaves' combined LR and hypothetical posterior) purely for display.
  The root prior is the only prior used; internal nodes do not have
